...
If the Pages project is not named `ai-smart-seo`, update the `name` value before using
...
`FIREBASE_PROJECT_ID` should be `codestap-9a0b2` for this app.

`ADMIN_EMAILS` is a comma-separated list of Firebase Authentication user emails
that can open `/admin`, for example `owner@example.com,partner@example.com`.
...
`https://app.example.com`. It may be omitted for initial testing because the
