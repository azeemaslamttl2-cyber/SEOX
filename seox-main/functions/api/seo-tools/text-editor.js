...
function apiError(error, headers) {
  ...
  const message = status >= 500 ? 'Text editor processing failed.' : error.message;
  ...
  return jsonResponse({
    ...
    message,
    error: {
      code: status === 401 ? 'UNAUTHORIZED' : status === 400 ? 'INVALID_REQUEST' : 'TEXT_EDITOR_ERROR',
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    const processed = transformTextEditor(input);
