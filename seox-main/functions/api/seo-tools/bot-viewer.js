...
import { viewAsBot } from '../../_lib/seo-tools.js';

const MAX_TOKEN_LENGTH = 512;

function apiError(error, headers) {
  const status = Number.isInteger(error?.status) ? error.status : 500;
  const message = status >= 500 ? 'Bot viewer analysis failed.' : error.message;
  ...
  return jsonResponse({
    ...
    data: null,
    error: {
      code: status === 401 ? 'UNAUTHORIZED' : status === 400 ? 'INVALID_REQUEST' : 'BOT_VIEWER_ERROR',
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    const data = viewAsBot(input);
    return jsonResponse({
      success: true,
      status: 'success',
      message: 'Bot viewer analysis completed successfully.',
      data,
      meta: {
        adminId: admin.id,
