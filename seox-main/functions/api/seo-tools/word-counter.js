...
function apiError(error, headers) {
  ...
  const message = status >= 500 ? 'Word count processing failed.' : error.message;
  ...
  return jsonResponse({
    ...
    data: null,
    error: {
      code: status === 401 ? 'UNAUTHORIZED' : status === 400 ? 'INVALID_REQUEST' : 'WORD_COUNTER_ERROR',
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    const processed = calculateWordCounter(input);
    return jsonResponse({
      success: true,
      status: 'success',
