...
const nav = [
  ...
  },
  {
    section: "Jira",
    items: [
      // Tickets is a page of its own because it is a working surface spanning
      // every project, not a setting.
      { label: "Tickets", to: "/jira/tickets", icon: Ticket },
      // Settings stays deep-linked to its tab rather than getting a route of
      // its own - the rule below about per-category settings routes applies to
      // it too. It earns a nav entry because, unlike the credential tabs, it
      // is a surface a user returns to: connection health, sync errors and the
      // retry queue all live there.
      { label: "Jira Settings", to: "/settings/general?tab=jira", icon: SquareKanban },
    ],
  },
  {
    section: "Settings",
